...
while ans[n - 1][m - 1][0] == -1 and ans[n - 1][m - 1][1] == -1:
    ...
    if mode:
        ...
        for k in range(1, 11):
            if y - k >= 0:
                s1 += int(f[y - k][x])
                if k >= 4: heapq.heappush(q, (heat + s1, y - k, x, not mode))
            if y + k < n:
                s2 += int(f[y + k][x])
                if k >= 4: heapq.heappush(q, (heat + s2, y + k, x, not mode))
    else:
        s1, s2 = 0, 0
        for k in range(1, 11):
            if x - k >= 0:
                s1 += int(f[y][x - k])
                if k >= 4: heapq.heappush(q, (heat + s1, y, x - k, not mode))
            if x + k < m:
                s2 += int(f[y][x + k])
                if k >= 4: heapq.heappush(q, (heat + s2, y, x + k, not mode))
